...
class HitBox
{
  ...
  is_touching(other, mask)
  {
    if (other instanceof HitBox && (!mask || mask && other.get_type() == mask || other.get_type() != this.#type))
    {
      ...
      let sec = this.#r + other.get_r();
      // Don't even bother with the expensive squaring calculations if we're too far away to begin with
      // Basically taking a page out of aabb
      if (dx > sec || dy > sec)
        return false;
      let d = dx * dx + dy * dy;
...
class TickableObject
{
  ...
  tick()
  {
    ...
    // Checks removability
    if (!((this instanceof Entity || this instanceof BGObject) && !this.bounds_delete) && (this.x < -256 || this.y < -256 || this.x >= width + 256 || this.y >= height + 256))
    {
      this.no_tick = true;
      this.deletable = true;
    }
...
{
  constructor(x, y, angle, speed, size, type, no_hitbox, force_spawn, spin, nograze, no_remove_on_hit)
  {
    super(x,y);
    // Can't spawn directly inside player
    if (this.get_distance(ply) <= 32 && !force_spawn)
    {
      this.deletable = true;
      return;
    }
  ...
  get_nograze() {return this.nograze;}
  set_nograze(yn) {this.nograze = yn;}
  
  #calc_velocities()
  {
    this.xsp = this.speed * Math.cos(this.angle * PI / 180);
    this.ysp = -this.speed * Math.sin(this.angle * PI / 180);
  }
  ...
  do_collisions()
  {
    ...
    {
      if (!this.nograze && this.hitbox.is_touching(ply.get_grazebox(),"graze"))
...
class Familiar extends Enemy
{
  constructor(parent,x,y,r,sprites,def_anim,hp,sp,r2)
  {
    super(x,y,r,sprites,def_anim,hp,sp,r2);
    this.parent = parent;
    this.bulletdie = true;
    if(!(parent instanceof Entity) || parent.is_dead())
      this.deletable = true;
  }
  
  get destroys_bullets() {return this.bulletdie;}
  set destroys_bullets(yn) {this.bulletdie = yn;}
  ...
  die(ignore_invin)
  {
    ...
    {
      if (this.bulletdie)
        if (this.hurtbox instanceof HitBox)
        {
          this.hurtbox.set_r(this.hurtbox.get_r() * 2);
          for (let b of bullets)
            if (this.hurtbox.is_touching(b.get_hitbox(),"bullet"))
              b.destroy();
        }
      super.die(ignore_invin);
    }
  }
  
  check_parent()
  {
    if(!this.parent || this.parent.is_dead())
      this.die(true);
    else if(this.parent.can_remove())
      this.destroy();
  }
...
class Boss extends Enemy
{
  ...
  die(ignore_invin, no_die_eff, no_death_anim, no_bullet_clear)
  {
    if (this.dead || this.deletable)
      return;
    if (ignore_invin || !this.is_invincible())
...
class Player extends Entity
{
  ...
  graze(bullet,size)
  {
    ...
    let a = random() * 2 * PI;
    p.set_xsp(6 * Math.cos(a));
    p.set_ysp(6 * Math.sin(a));
  ...
  movement()
  {
    ...
    if (this.can_move)
    {
      let temp_x = speed * Math.cos(angle * PI / 180);
      let temp_y = -speed * Math.sin(angle * PI / 180);
      if (temp_x > 1)
  ...
  specials()
  {
    ...
    if (this.vars.sp_atk_timer > 0)
    {
      ...
      {
        let temp_y = Math.sin(this.vars.sp_atk_timer / 15 * PI) * 32;
        new PlyShot(this.x + 22,this.y + temp_y);
        new PlyShot(this.x + 22,this.y - temp_y);
        temp_y = Math.cos(this.vars.sp_atk_timer / 15 * PI) * 96;
        new PlyShot(this.x + 22,this.y + temp_y);
        new PlyShot(this.x + 22,this.y - temp_y);
      }

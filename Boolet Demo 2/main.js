...
function main_loop()
{
  ...
  bg_tick_objs = bg_tick_objs.filter((t) => t != null && !t.can_remove());
  tick_objects = tick_objects.filter((t) => t != null && !t.can_remove());
  entities = entities.filter((e) => e != null && !e.can_remove());
  bullets = bullets.filter((b) => b != null && !b.can_remove());
  ply_bullets = ply_bullets.filter((b) => b != null && !b.can_remove());
  ...
  //print(tick_objects.length + bg_tick_objs.length);
  push();
  let fps = frameRate();
  fill(255);
  stroke(0);
  text(fps.toFixed(1), 10, height - 10);
  pop();
}
